...
public class PickUpFromPage extends AbstractPageClass{
    // call the configloader to get the values we want to input
    private ConfigLoader configLoader;

    public PickUpFromPage(IOSDriver driver) {
        super(driver);
    ...
    @iOSXCUITFindBy(xpath = "//XCUIElementTypeStaticText[@value='Sender name']/following::XCUIElementTypeTextField[1]")
    ...
    public boolean enterEstateNumber(){
        try{
            ...
            String estateStreetNumber = configLoader.getProperty("ESTATE_NUMBER_SENDER");
            ...
            Thread.sleep(2000);
            System.out.println("The first package FROM address text is: " + dynamicTextOptionVisible.getText());
    ...
    public boolean enterRoomAndFloorDetails(){
        try{
            ...
            String roomAndFloorDetails = configLoader.getProperty("ROOM_AND_FLOOR_SENDER");
    ...
    public boolean enterSenderName(){
        try{
    ...
    public boolean enterPhoneNumber(){
        try{
            String phoneNumber = configLoader.getProperty("SENDER_PHONE_NUMBER");
